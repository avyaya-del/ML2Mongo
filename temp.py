...
def convert_xquery_to_mongodb(xquery_command):
    ...
    patterns = {
        'create': re.compile(r'xdmp:document-insert{{{{\("([\^"]+)", (.+)\)}}}}'),
        'read': re.compile(r'fn:doc{{{{\("([\^"]+)"\)}}}}'),
        'update': re.compile(r'xdmp:node-replace{{{{\("([\^"]+)", (.+)\)}}}}'),
        'delete': re.compile(r'xdmp:document-delete{{{{\("([\^"]+)"\)}}}}')
    }
    ...
    for operation, pattern in patterns.items():
        match = pattern.match(xquery_command)
        if match:
